...
for d in ${CORPORA[@]}; do
  ...
  if [[ -n "${UD_DATA[$d]}" ]]; then  # Convert gold UD to UCCA and gold UCCA to UD
    ...
    python -m semstr.convert "${DATA[$d]}/*.xml" -o $PARSED/conllu/ucca/$d -f conllu --label-map=$UCCA_UD_LABEL_MAP $*
    for MODEL in ${UCCA_UD_MODEL[$d]} ${UCCA_MODEL[$d]}; do  # Parsed UCCA to UD
      python -m semstr.convert "$PARSED/xml/$MODEL/$d/*.xml" -o $PARSED/conllu/$MODEL/$d -f conllu --label-map=$UCCA_UD_LABEL_MAP $*
    done
  fi
done
