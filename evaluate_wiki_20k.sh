...
for d in ${CORPORA[@]}; do
  ...
  for MODEL in ${UCCA_UD_MODEL[$d]} ${UCCA_MODEL[$d]} ${UDPIPE_MODEL[$d]} ${UD_MODEL[$d]}; do
      ...
      python -m semstr.evaluate $GUESSED $REF --constructions=categories -s $OUT/ud/$MODEL.csv -o $OUT/all/ud/$MODEL.csv --ref-yield-tags=$PARSED/conllu/${UDPIPE_MODEL[$d]}/$d.conllu -q $*
  done
done
